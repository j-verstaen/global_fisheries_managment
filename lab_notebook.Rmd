...
itq <- ITQ_projection %>%
  select("Country", "Year", "CommName", "SciName", "SpeciesCat", "CatchShare", "yearitq", "itq_now","iq", "itq", "ivq", "turf", "programstart")
...
colnames(edf_turf_simple) <- c("Country", "programstart", "SciName")

##want to merge the edf simple with ITQ/Projections but maybe wait for Gina to respond with her TURF data
##in ITQ data says only one species of fish in japan has TURFs but not true

edf_projection_1 <- join(edf_turf_simple, itq, by = c("Country", "programstart", "SciName"), type = "full")

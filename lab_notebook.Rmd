...
fisheries_with_turfs <- corbette_fisheries %>%
  filter(turf != "FALSE")

fisheries_with_itq <- corbette_fisheries %>%
   filter(itq == "TRUE" | iq == "TRUE" | ivq == "TRUE" )

```

reupdating upside data with new ITQ list

###JULIETTE: check why the fisheries recent only has FALSE for turfs and no TRUEs
...
## adding turf information to upside projection (use fisheries recent?)
```{r}
...
turf_only <- turf_itq_isscaap %>%
  ...
  filter(SpeciesCat != 91) %>%
  filter(SpeciesCat != 92)
...
fisheries_recent_generousturf_1 <- fisheries_recent %>%
  ...
  filter(SpeciesCat.x != 91) %>%
  filter(SpeciesCat.x != 92)

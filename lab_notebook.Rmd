...
#RAM data wrangling
```{r}
ram <- select(timeseries_values_views, "stocklong", "year", "TBbest", "TCbest", "BdivBmsypref", "UdivUmsypref")

colnames(ram) <- c( "CommName", "Year", "Biomass", "Catch", "BvBmsy", "FvFmsy")
...
projectramcheck <- join_project_ram %>%
  filter(Year == "2013") %>%
  select("IdOrig", "Year", "CommName")
...
unite_ram_commname <- ram %>% 
  unite (name_year, CommName, Year, sep = "-")
join_project_ram_2 <- join(unite_projection_commname, unite_projection_commname, by = "name_year", type = "full")

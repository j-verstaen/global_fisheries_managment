...
projection_data_fig1$CatchShare <- as.factor(projection_data_fig1$CatchShare)
...
projection_data_fig2 <- readRDS(here("data","projection_data.rds")) %>% 
  ...
  filter(Dbase != "SOFIA") %>%
  filter(IdLevel != "Neis") %>%
  select( IdOrig, Country, CatchShare) 
  
colnames(projection_data_fig2) <- c("IdOrig", "CNTRY_NAME","CatchShare")
...
projection_spatial <- dplyr::left_join(projection_data_fig2, countries, by = "CNTRY_NAME")
projection_spatial <- st_as_sf(projection_spatial) 

fig_2 = ggplot() +
  geom_sf(data= projection_spatial, color = CatchShare) 


fig_2
#plot(projection_spatial)

#tm_shape(countries) +
 # tm_polygons() +
  #tm_shape(projection_spatial, colour = CatchShare) 

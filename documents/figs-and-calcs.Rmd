...
all_turfs <- dplyr::full_join(d_turfs, edf_turf, by= c("Country", "SciName", "CatchShare")) %>%
  distinct()
...
projection_turfs <- full_join(projection_data, turfs, by = c("Country", "SciName", "SpeciesCat","Year"), all = T) %>% 
...
catch_shares <- left_join(projection_turfs, gdp, by = "Country") %>% 
  group_by(IdOrig) %>%
...
catch_shares_and_turfs <- left_join(projection_turfs, gdp, by = "Country") %>% 
  group_by(IdOrig) %>%
  dplyr::mutate(max_year = max(Year)) %>% 
  filter(max_year== 2012,
         CatchShare %in% c(1,2),
...
everything <- left_join(projection_turfs, gdp, by = "Country") %>% 
  group_by(IdOrig) %>%
  dplyr::mutate(max_year = max(Year)) %>% 
  filter(max_year== 2012,
         Year == max_year,
...
upjoin <-
  caret::upSample(
    x = select(catch_shares_and_turfs, -CatchShare),
    y = factor(catch_shares_and_turfs$CatchShare),
    yname = "CatchShare"
  ) 
...
g <- pROC::roc(CatchShare ~ pred, data = orig_cs_or_turf_fit)

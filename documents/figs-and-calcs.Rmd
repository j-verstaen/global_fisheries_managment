...
projection_data_mutate <- projection_data  %>%
...
join_mutate <- left_join(projection_data_mutate, gdp, by = "Country") %>% 
...
itq_broad_fit <-
  glm(
    formula = CatchShare ~ gdp_center + MainCat,
    family = "binomial",
    data = downjoin
  )
...
aug_itq_broad_fit %>% 
  ggplot(aes(CatchShare, pred, group = CatchShare)) + 
  geom_boxplot()
...
plot(bayes_itq_broad_fit)

pps = rstanarm::posterior_predict(bayes_itq_broad_fit) %>% 
  colMeans()

aug_itq_broad_fit <- broom::augment(bayes_itq_broad_fit) %>% 
  mutate(pred = pps)
